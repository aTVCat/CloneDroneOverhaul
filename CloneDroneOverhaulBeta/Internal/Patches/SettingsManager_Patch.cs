...
namespace CDOverhaul.Patches
{
    ...
    internal static class SettingsManager_Patch
    {
        ...
        private static void SetVsyncOn_Prefix(bool value)
        {
            ...
            if (value)
            {
                SettingInfo.SavePref(SettingsController.GetSetting("Graphics.Settings.Target framerate", true), 2);
            }
            else
            {
                SettingInfo.SavePref(SettingsController.GetSetting("Graphics.Settings.Target framerate", true), 0);
            }
        }
    }
}
