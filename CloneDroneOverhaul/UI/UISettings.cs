...
{
    public class UISettings : V3_ModHUDBase
    {
        ...
        private class UISettingEntry : MonoBehaviour, UnityEngine.EventSystems.IPointerEnterHandler, UnityEngine.EventSystems.IPointerExitHandler
        {
            ...
            {
                UISettings.GetInstance<UISettings>().ShowAdditionalSettings(Entry);
            }
                ...
                if (_isMouseIn)
                {
                    ...
                    if (Input.GetKeyDown(KeyCode.Q))
                    {
                        if (OverhaulMain.GetSetting<object>(Entry.ID) != Entry.DefaultValue)
                        {
                            ...
                            notif.Send();
                            UISettings.GetInstance<UISettings>().RefreshPage();
                        }
                    }
                }
                if (_isMouseDown)
        ...
        private class CategoryButton : MonoBehaviour
        {
            ...
            private void OnButtonClicked()
            {
                ...
                if (isCategory)
                {
                    UISettings.GetInstance<UISettings>().SelectCategory(str);
                }
                else
                {
                    UISettings.GetInstance<UISettings>().SelectSection(str);
                }
            }
        }
    }
