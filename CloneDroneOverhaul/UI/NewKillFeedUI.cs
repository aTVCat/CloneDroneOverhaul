...
namespace CloneDroneOverhaul.UI
{
    public class NewKillFeedUI : ModGUIBase
    {
        ...
        List<KillEventData> events = new List<KillEventData>();

        public static NewKillFeedUI Instance;

        float timeToCheckNextEvent = 0.7f;
        ...
        void checkEvent(KillEventData evnt)
        {
            ...
            {
                bool isFire = false;
                PlayerUpgradeCollection coll = CharacterTracker.Instance.TryGetLivingCharacterWithPlayFabID(evnt.KillerID).GetRobotInfo().UpgradeCollection as PlayerUpgradeCollection;
                switch (type)
                {
        ...
        void removeObj(GameObject gObj)
        {
