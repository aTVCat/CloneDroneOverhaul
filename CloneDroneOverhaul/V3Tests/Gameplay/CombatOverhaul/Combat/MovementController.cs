...
namespace CloneDroneOverhaul.V3Tests.Gameplay
{
    public class MovementController : FirstPersonMoverAddititonBase
    {
        ...
        void Update()
        {
            ...
            if (!isMoving)
            {
                _speedMultipler = Mathf.Clamp(_speedMultipler - Time.deltaTime * 6f * Time.timeScale, 0f, 1f + _sprint);
            }
            else
            {
                _speedMultipler = Mathf.Clamp(_speedMultipler + Time.deltaTime * 4f * Time.timeScale, 0f, 1f + _sprint);
            }
