...
{
    public class OverhaulCacheAndGarbageController
    {
        private static bool _hasPrepared;

        private static Dictionary<string, object> _preparedStuff;
        ...
        public static void PrepareStuff()
        {
            ...
            _preparedStuff = new Dictionary<string, object>
            {
                ...
                { "Dark_Wall1_LOD2", AssetLoader.GetObjectFromFile<GameObject>("overhaul_lods", "Dark_Wall1_LOD1") },
        ...
        public static void CacheObject<T>(in T @object, in string name)
        {
            if (!HasCached(name))
            {
                _preparedStuff.Add(name, @object);
            }
        }
        ...
        {
            if (_preparedStuff.ContainsKey(id))
            {
                object rawObj = _preparedStuff[id];
                if (!(rawObj is T))
                {
                    Modules.ModuleManagement.ShowError("Object with ID: " + id + " is not " + typeof(T).ToString());
                }
                T obj = (T)_preparedStuff[id];
                return obj;
            }
            else
            {
        ...
        {
            return _preparedStuff.ContainsKey(id);
        }
        ...
        public static string SaveImageOnDisk(in Texture2D texture, in string filename)
        {
            return V3Tests.Base.ModDataController.SaveTempAssetOnDisk(V3Tests.Utilities.OverhaulUtilities.TextureAndMaterialUtils.TextureToByteArray(texture), filename);
        }
