...
{
    public class UISettings_OLD : V3_ModHUDBase
    {
        ...
        public void Show()
        {
            ...
            populateSettings(true, Modules.OverhaulSettingsManager_Legacy.Instance.GetAllSettings());
        ...
        {
            populateSettings(false, OverhaulSettingsManager_Legacy.Instance.GetSettings(category, section));
        }
        ...
        {
            selectedSection = str;
            populateSettings(false, OverhaulSettingsManager_Legacy.GetInstance<OverhaulSettingsManager_Legacy>.GetSettings(selectedCategory, selectedSection));
        }
        public void ShowAdditionalSettings(OverhaulSettingsManager_Legacy.SettingEntry entry)
        {
            ChildrenSettings.gameObject.SetActive(true);
            TransformUtils.DestroyAllChildren(AdditSettingsContainer);
            List<OverhaulSettingsManager_Legacy.SettingEntry> list = OverhaulSettingsManager_Legacy.Instance.GetSettings(entry.ChildSettings.ChildrenSettingID);
            foreach (OverhaulSettingsManager_Legacy.SettingEntry entry2 in list)
            {
        ...
        private void searchSettings(string str)
        {
            ...
            {
                populateSettings(false, Modules.OverhaulSettingsManager_Legacy.Instance.GetSettings(selectedCategory, selectedSection));
            }
            populateSettings(false, Modules.OverhaulSettingsManager_Legacy.Instance.SearchSettings(str));
        }
        ...
        private void populateSections()
        {
            ...
            {
                List<Modules.OverhaulSettingsManager_Legacy.SettingEntry.CategoryPath> paths = Modules.OverhaulSettingsManager_Legacy.Instance.GetPaths(selectedCategory);
                foreach (Modules.OverhaulSettingsManager_Legacy.SettingEntry.CategoryPath pathhh in paths)
                {
                    if (!spawnedSections.Contains(pathhh.Section))
                    {
                        ModdedObject mObj = Instantiate<ModdedObject>(SectionItemPrefab, SectionContainer);
                        mObj.GetObjectFromList<Text>(0).text = OverhaulMain.GetTranslatedString("Settings_Section_" + Modules.OverhaulSettingsManager_Legacy.Instance.GetSectionName(pathhh.Section));
        ...
        private void populateSettings(bool onlyCategories, List<Modules.OverhaulSettingsManager_Legacy.SettingEntry> settingsList)
        {
            List<Modules.OverhaulSettingsManager_Legacy.SettingEntry> settings = settingsList;
            ...
            Modules.OverhaulSettingsManager_Legacy.SettingEntry.CategoryPath path = Modules.OverhaulSettingsManager_Legacy.Instance.GetPageData(selectedCategory, selectedSection);
            if (!path.IsEmpty)
            {
            ...
            foreach (Modules.OverhaulSettingsManager_Legacy.SettingEntry entry in settings)
            {
                if (!entry.ForceHide && onlyCategories)
                {
                    if (!spawnedCategories.Contains(entry.Path.Category))
                    {
                        ...
                        mObj.GetObjectFromList<Text>(0).text = OverhaulMain.GetTranslatedString("Settings_Category_" + Modules.OverhaulSettingsManager_Legacy.Instance.GetCategoryName(entry.Path.Category));
        ...
        private void populateSetting(Modules.OverhaulSettingsManager_Legacy.SettingEntry entry, Transform parent, bool ignoreIsHidden)
        {
            if (entry.ForceHide)
            {
                return;
            ...
            if (entry.Type == typeof(bool))
            {
                ...
                setting.gameObject.AddComponent<UISettingEntry>().SetupValue(entry.Type, entry, CloneDroneOverhaulDataContainer.SettingsData.GetSettingValue(entry.ID));
            }
            ...
            else if (entry.Type.IsEnum)
            {
                ...
                setting.gameObject.AddComponent<UISettingEntry>().SetupValue(entry.Type, entry, CloneDroneOverhaulDataContainer.SettingsData.GetSettingValue(entry.ID), entry.ValueSettings.DropdownEnumType);
            }
            if (setting == null)
            {
        ...
        private class UISettingEntry : MonoBehaviour, UnityEngine.EventSystems.IPointerEnterHandler, UnityEngine.EventSystems.IPointerExitHandler
        {
            ...
            private Modules.OverhaulSettingsManager_Legacy.SettingEntry Entry;
            private System.Type Type;
            private Modules.OverhaulSettingsManager_Legacy.SettingEntry.UIValueSettings Settings;
            private bool _isMouseIn;
            private bool _isMouseDown;

            private float _floatValueWaitingToSet;

            public void SetupValue(Type type, Modules.OverhaulSettingsManager_Legacy.SettingEntry sEntry, object value, Type enumerator = null)
            {
                Entry = sEntry;
            ...
            {
                UISettings_OLD.GetInstance<UISettings_OLD>().ShowAdditionalSettings(Entry);
            }

            private void ToggleBoolValue(bool val)
            {
                CloneDroneOverhaulDataContainer.SettingsData.SaveSetting(Entry.ID, val, false);
            }
            ...
            {
                CloneDroneOverhaulDataContainer.SettingsData.SaveSetting(Entry.ID, num, false);
            }
            ...
            private void Update()
            {
                ...
                if (_isMouseIn)
                {
                    ...
                    {
                        BaseUtils.CopyToClipboard(MyModdedObject.GetObjectFromList<InputField>(2).text, true);
                    }
                    if (Input.GetKeyDown(KeyCode.Q))
                    {
                        if (OverhaulMain.GetSetting<object>(Entry.ID) != Entry.DefaultValue)
                        {
                            CloneDroneOverhaulDataContainer.SettingsData.SaveSetting(Entry.ID, Entry.DefaultValue, false);
                            SNotification notif = new SNotification("Setting reset!", "Set default value for " + OverhaulMain.GetTranslatedString(Entry.NameLocalizationID) + " (" + Entry.DefaultValue.ToString() + ")", 2f, UINotifications.NotificationSize_Default, null);
                            notif.Send();
                            UISettings_OLD.GetInstance<UISettings_OLD>().RefreshPage();
                        }
                    }
                }
                if (_isMouseDown)
        ...
        private class CategoryButton : MonoBehaviour
        {
            ...
            private void OnButtonClicked()
            {
                ...
                if (isCategory)
                {
                    UISettings_OLD.GetInstance<UISettings_OLD>().SelectCategory(str);
                }
                else
                {
                    UISettings_OLD.GetInstance<UISettings_OLD>().SelectSection(str);
                }
            }
        }
    }
