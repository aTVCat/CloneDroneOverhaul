...
{
    public class UIPauseMenu : V3_ModHUDBase
    {
        ...
        private void Awake()
        {
            ...
            //CDOSettings.onClick.AddListener(UISettings_OLD.GetInstance<UISettings_OLD>().Show);
        ...
        private void refreshPlayers()
        {
            ...
            for (int i = 0; i < allPlayerInfoStates.Count; i++)
            {
                ...
                if (!infoState.IsDetached() && infoState.ShouldBeVisibleToPlayers() && !(infoState == Singleton<MultiplayerPlayerInfoManager>.Instance.GetLocalPlayerInfoState()))
                {
                    ...
                    {
                        BaseUtils.CopyToClipboard(mObj.GetObjectFromList<Text>(9).text, true);
                    });
        ...
        private void refreshWorkshop()
        {
            ...
            if (item != null)
            {
                ...
                V3.Utilities.Coroutines.LoadWorkshopImage(item.PreviewURL, item.WorkshopItemID.ToString(), delegate (Sprite spr)
                {
        ...
        private void onMainMenuClicked() // Implement Generic2Button
        {
            ...
            OverhaulMain.Timer.CompleteInTime(resetMainMenuButton, 3, true);
        }
        private void onExitGameClicked()
        {
            ...
            OverhaulMain.Timer.CompleteInTime(resetExitGameButton, 3, true);
        }
        ...
        {
            BaseUtils.CopyToClipboard(CUI_Code.text, true);
        }
