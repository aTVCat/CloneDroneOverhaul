    ...
    private bool ProcessDriveDownload()
    {
        ...
        if (formIndex >= 0)
        {
            ...
            while ((inputIndex = content.IndexOf("<input type=\"hidden\"", inputIndex + 10)) >= 0 && inputIndex < formEndIndex)
            {
                ...
                _ = sb.Append(isFirstArgument ? '?' : '&').Append(content, linkIndex, content.IndexOf('"', linkIndex) - linkIndex).Append('=');

                linkIndex = content.IndexOf("value=", linkIndex) + 7;
                _ = sb.Append(content, linkIndex, content.IndexOf('"', linkIndex) - linkIndex);
