...
{
    public class RealisticSkyboxesAddon : GameAddon
    {
        ...
        private IEnumerator loadAllSkyboxesCoroutine(AssetBundle bundle)
        {
            if (ModAdvancedCache.TryGet("RealisticSkyboxes", out Material[] array))
            {
                ...
                GlobalEventManager.Instance.AddEventListener(GlobalEvents.LightSettingsRefreshed, refreshSkybox);
                yield break;
            }
            ...
            {
                Skyboxes = new Material[r.allAssets.Length];
                int i = 0;
                foreach (Object obj in r.allAssets)
                {
                    Skyboxes[i] = r.allAssets[i] as Material;
                    i++;
                }
            }
            ModAdvancedCache.Add("RealisticSkyboxes", Skyboxes);
            refreshSkybox();
            ContentManager.Instance.SetContentIsLoading(this, false);
            GlobalEventManager.Instance.AddEventListener(GlobalEvents.LightSettingsRefreshed, refreshSkybox);
            yield break;
