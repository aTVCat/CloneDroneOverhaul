...
{
    public static class ModUGCUtils
    {
        /// <summary>
        ...
        public static void Debug_RequestWorkshopItem(SteamWorkshopItem steamWorkshopItem)
        {
            Debug_Items = null;
            Debug_Error = null;
            Debug_LastQueryIsCached = false;
            Debug_LastQueryResults = -1;
            Debug_LastQueryMatchingResults = -1;
            _ = GetWorkshopItem(steamWorkshopItem.WorkshopItemID, Debug_RequestParameters, delegate (SteamWorkshopItem item)
            {
                Debug_Item = item;
            }, delegate (string error)
            {
        ...
        public static bool GetWorkshopItem(PublishedFileId_t publishedFileId, RequestParameters requestParameters, Action<SteamWorkshopItem> callback, Action<string> errorCallback)
        {
            if (publishedFileId == default || requestParameters == null)
                return false;

            UGCQueryHandle_t queryHandle = createWorkshopItemRequest(publishedFileId);
            requestParameters.ConfigureQuery(queryHandle);
            doRequestWorkshopItems(queryHandle, delegate (SteamUGCQueryCompleted_t queryResult, bool io)
            {
                onQueryCallback_OneItem(queryResult, io, callback, errorCallback);
            });
            return true;
        }
        ...
        private static UGCQueryHandle_t createWorkshopItemRequest(PublishedFileId_t publishedFileId)
        {
            return SteamUGC.CreateQueryUGCDetailsRequest(new PublishedFileId_t[] { publishedFileId }, 1);
        }
